...
def main(args=None):
    ...
    steering_angle_publisher = control_publisher_node.create_publisher(Float32, '/steering_angle', 10)
    ...
    try:
        while rclpy.ok():
            ...
            if path_points:
                ...
                # 타겟 및 조향각 플로팅
                cv2.circle(img, target, 5, (0, 0, 255), -1)
                cv2.line(img, CAR_POSITION, target, (0, 255, 0), 4)
                
                cv2.imshow("PATH PLAN AND CONTROL", img)
                cv2.waitKey(1)
